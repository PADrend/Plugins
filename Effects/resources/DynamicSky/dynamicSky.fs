...
void main(void){
    ...
    if(noCloudsNearHorizon>0.20){
        ...
        float sunBlooming = pow( max(0.0, dot(-lightDir,norm)), bloomingExponent )*bloomingScale*(1.0-cloudDensity);
...
//        float cloudBrightness = clamp( pow( clamp(dot(cNorm, lightDir),0.0,1.0), 2.0) * 1.0 + 0.6 + sunBlooming, 0.0, 2.0);
        ...
        skyColor = mix( skyColor, cloudColor.rgb  * cloudBrightness,
                       pow(clamp((cloudDepth-(1.0-cloudDensity))*4.0,0.0,noCloudsNearHorizon),3.0 )); //params
    }
    ...
    // ---------------------------
    // -- add sun
    skyColor += vec3(pow( max(0.0, dot(-lightDir,norm)), 1024.0 * (1.0-hazeFactor*0.95) ));

//	
//	if(mod(skyPos_ws.x+0.001,0.1)<0.002 || mod(skyPos_ws.y+0.001,0.1)<0.002 )
//		skyColor *= 2.0;
