 ...
 */
/****
 **	[Plugin:NodeEditor] NodeEditor/Tools/TreeMenu.escript
 **/

NodeEditorTools.registerMenues_TreeTools := fn() {
	...
	gui.registerComponentProvider('NodeEditor_TreeToolsMenu.treeOperations',[
		...
		},
		{
			...
			GUI.ON_CLICK:fn(){
                ...
                popup.addAction("Remove", (fn(dataWrap,states){
                        foreach(states as var id, var state){
                    ...
                    }).bindLastParams(dataWrap,states)
                );
		...
		"*Tree building op.*",
		{
			...
			GUI.ON_CLICK:fn(){
				...
				popup.addOption({
					...
					GUI.ON_DATA_CHANGED	:	((fn(data, options){options.func = data;}).bindLastParams(options)),
				...
				popup.addOption({
					...
					GUI.ON_DATA_CHANGED	:	((fn(data, options){options.map["MAX_TREE_DEPTH"] = data;}).bindLastParams(options)),
				...
				popup.addOption({
					...
					GUI.ON_DATA_CHANGED	:	((fn(data, options){options.map["MAX_CHILD_COUNT"] = data;}).bindLastParams(options)),
				...
				popup.addOption({
					...
					GUI.ON_DATA_CHANGED	:	((fn(data, options){options.map["LOOSE_FACTOR"] = data;}).bindLastParams(options)),
				...
				popup.addOption({
					...
					GUI.ON_DATA_CHANGED	:	((fn(data, options){options.map[MinSG.TreeBuilder.PREFERE_CUBES] = data;}).bindLastParams(options)),
				...
				popup.addOption({
					...
					GUI.ON_DATA_CHANGED	:	((fn(data, options){options.map[MinSG.TreeBuilder.USE_GEOMETRY_BB] = data;}).bindLastParams(options)),
				...
				popup.addOption({
					...
					GUI.ON_DATA_CHANGED	:	((fn(data, options){options.map[MinSG.TreeBuilder.EXACT_CUBES] = data;}).bindLastParams(options)),
