...
void main() {
	...
	for(int i = 0; i < sg_lightCount; ++i){
		...
		if (light.NdotL > 0.0 || surface.NdotV > 0.0) {
			...
			diffuse += diffuseBrdf * light.intensity.rgb * light.NdotL;
			...
			specular += specularBrdf * light.intensity.rgb * light.NdotL;
		}
	}
	vec3 color = diffuse + specular + material.emissive;

	// TODO: use define to enable/disable masking (otherwise early-depth test is not possible)
	if(sg_pbrAlphaMode == ALPHA_MODE_MASK) {
