...
Tool.onNodesSelected_static += fn(Array selectedNodes){
  ...
  // Create path
	var pathVisuNode = new MinSG.GeometryNode(createPathMesh(node));
  ...
  var wpIndex = -1;
    ...
    var waypointName = "" + (++wpIndex) + " - " + waypoint.getTime();
		...
		ctxt.pathVisuNode := pathVisuNode;
  ...
  var wpIndex = -1;
	...
	entries += {
		...
		GUI.MENU : [node] => fn(node){
			...
			var index = new Std.DataWrapper(node.getWaypoints().count()-1);
			var dir = new Std.DataWrapper(false);
			return [
				"Add after:",
				{
					GUI.TYPE : GUI.TYPE_NUMBER,
					GUI.DATA_WRAPPER : index,
				},
				/*{
					GUI.TYPE : GUI.TYPE_BUTTON,
					GUI.LABEL : "Create",
